classdef FullStimuliPreview < symphonyui.core.ProtocolPreview
    % Displays cell array of stimuli on a 2D plot
    % 
    % Stimuli are concatenated along the x-axis to show full epoch group
    % 
    % History:
    %   17Feb2019 - SSP
    % ---------------------------------------------------------------------
    ...
    methods 
        function obj = FullStimuliPreview(panel, createStimuliFcn, colors)
            ...
            if nargin < 3
                ...
                assert(length(colors) == 3, 'Colors must be Nx3 matrix');
                obj.colors = colors;
            end
